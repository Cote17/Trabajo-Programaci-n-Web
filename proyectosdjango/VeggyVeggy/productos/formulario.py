...
class NewUserForm(ModelForm):
    ...
    #usuario = forms.CharField(widget=forms.TextInput(attrs={'class':'formulario__input','id':'usuario','name':'usuario'}))
    correo = forms.CharField(widget=forms.EmailInput(attrs={'class':'formulario__input','id':'correo','name':'correo'}))
    telefono = forms.CharField(widget=forms.TextInput(attrs={'class':'formulario__input','id':'telefono','name':'telefono'}))
    terminos = forms.BooleanField(widget=forms.CheckboxInput(attrs={'class':'formulario__checkbox','id':'terminos','name':'terminos'}))

    class Meta:
        model = NewUser
        fields = ['nombre','telefono','correo','password','password2','terminos']#,'usuario']
